...
const client = new FetchClient({ baseURL: 'https://funding-pips-live-agent.onrender.com' });
...
export const useLiveAgent = (runtime: any) => {
  ...
  const subscribeToConversation = (platform: LiveAgentPlatform, userID: string, conversationID: string) => {
    socketRef.current = new WebSocket(
      `wss://funding-pips-live-agent.onrender.com/${platform}/user/${userID}/conversation/${conversationID}/socket`
    );
  ...
  const sendUserReply = async (content: string[], attachmentUrls) => {
    console.log(attachmentUrls);
    // console.log(content);
    // const fileTest = 'https://drive.google.com/uc?export=download&id=15MwAlQ4Y7SUPZ0kw7Fmz8vS8s1RywscF';

    if (typeof content === 'string') {
      runtime.addTurn({ ...createTurn(TurnType.USER), message: content });
      socketRef.current?.send(JSON.stringify({ type: SocketEvent.USER_MESSAGE, data: { message: content, attachmentUrls } }));
    }
  };
